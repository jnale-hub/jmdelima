...
export default function TitleList({ titles }: { titles: string[] }) {
    ...
    return (
        <ul className="flex flex-wrap gap-2 w-full justify-center lg:justify-start">
            {titles.map((title, index) => (
                <motion.li
                    ...
                    initial="hidden"
                    animate="visible"
                    className={`text-xs font-bold ${textColors[index % textColors.length]}  p-1 px-2 rounded-full flex items-center`}
                >
