...
export default function ContactForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    const response = await sendMail(values.name, values.email, values.message);
    setIsLoading(false);
    ...
    if (response && response.success) {
      ...
      toast.error(response.message, {
        description:
          'Please try again later, or not, because this person is cheap AF, just email me! Thanks!',
        duration: 5000,
        position: isMobile ? 'top-center' : 'bottom-right',
        classNames: {
          icon: 'text-red-600',
